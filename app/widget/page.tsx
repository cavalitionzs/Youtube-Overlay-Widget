  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100vh", background: "#000", overflow: "hidden" }}>
      ...
      {!current && (
        <div style={idleStyle}>
          <YoutubeMark />
          <span>Waiting for a song request — type !request &lt;YouTube link&gt;</span>
        </div>
      )}
        ...
        <div style={overlayStyle}>
          ...
          {upcoming.length > 0 && (
            <div style={queueStyle}>
              Up next: {upcoming.slice(0, 3).map((v) => v.title).join(" · ")}
            </div>
          )}
        </div>
      )}
...
function YoutubeMark() {
  return (
    <svg viewBox="0 0 24 24" width={22} height={22} style={{ opacity: 0.9, flex: "none" }}>
